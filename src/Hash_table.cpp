...
int Hash_table_ctor(struct Hash_table *const ht, hash_func hash, void *const gen, size_t const gen_len) {
	assert(ht);
	...
	ht->buckets_cnt	= HT_INITIAL_BUCKETS;
...
static int list_insert(List_node **const cur, ht_key_t const key) {
	assert(cur);
...
int Hash_table_insert(struct Hash_table *const ht, ht_key_t const key) {
	assert(ht);
...
static byte_t list_find(List_node const *const cur, ht_key_t const key) {
	if (!cur)		{ return 0; }
	if (HT_KEY_EQUAL(cur->key, key)) { return 1; }
	return list_find(cur->next, key);
}

byte_t Hash_table_find(struct Hash_table const *const ht, ht_key_t const key) {
	assert(ht);
